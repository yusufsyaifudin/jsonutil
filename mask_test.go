...
func TestMask(t *testing.T) {
	type TestCase struct {
		Name       string
		Input      string
		MaskedKeys map[string]jsonutil.MaskFunc
	...
	for _, tc := range testCases {
		t.Run(tc.Name, func(t *testing.T) {
			mask := jsonutil.NewMasking(jsonutil.Config{Keys: tc.MaskedKeys})
			out, err := mask.MaskByte(context.Background(), []byte(tc.Input))
	...
	mask := jsonutil.NewMasking(config)
	// by := []byte(`["ini", {"nest": ["hello", {"foo": {"hello": "world"}}]}]`)
	by := []byte(`[{"str": {"nest": "ini string"}, "nest": "hello", "nesting": ["ini", "string"]}]`)
...
func BenchmarkMasking_Mask(b *testing.B) {
	...
	mask := jsonutil.NewMasking(jsonutil.Config{Keys: map[string]jsonutil.MaskFunc{
		"avatar": nil,
	}})
